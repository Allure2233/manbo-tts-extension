...
const volcKey=document.getElementById('volcKey'),volcTitle=document.getElementById('volcTitle'),volcBody=document.getElementById('volcBody');
const voiceBtns=document.querySelectorAll('.voice-btn');
let currentVoice='manbo';
...
// 加载
sg(['enabled','volume','speed','delay','volcano_key','current_voice']).then(r=>{
  ...
  setVoiceUI(currentVoice);updateUI(tb.checked);updateLabels();
});
...
function setVoiceUI(v){
  ...
  st.textContent=tb.checked?'已启用 · '+({manbo:'曼波',jelpeta:'杰尔佩塔'}[v]||v):'已暂停';
}
...
volcTitle.addEventListener('click',()=>{volcTitle.classList.toggle('collapsed');volcBody.style.display=volcTitle.classList.contains('collapsed')?'none':''});

function updateUI(en){
  if(en){sd.className='dot';st.textContent=volcKey.value?'已启用 · '+({manbo:'曼波',jelpeta:'杰尔佩塔'}[currentVoice]):'已启用 · 中转站'}
